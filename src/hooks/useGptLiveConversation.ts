...
import type { LiveConversationState, LivePhase } from "./useLiveConversation";
...
export function useGptLiveConversation(
  ...
  }, [enabled, finish, onAssistantRequested, onError, releaseBrowserMedia]);
  ...
  const start = useCallback(async () => {
    ...
    commandDetectorRef.current.reset();
    try {
      await invoke("prepare_live_session", { mode: "gpt-live-1" });
  ...
  }, [clearTimer, enabled, fail, finish, finishAfterLocalGoodbye, microphoneName, phase, registerRemoteSpeech, switchToDictation, updatePhase]);
