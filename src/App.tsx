...
function Dashboard({ data, language, isBusy, onOpenOnboarding, onTest, onRetry, onDeleteFailed, onCopy, onOpen, onRetranscribe }: {
  ...
  return (
    <div className="page dashboard">
      ...
      {data.failedRecording && <FailedCard failed={data.failedRecording} language={language} onRetry={onRetry} onDelete={onDeleteFailed} />}
      <section className="section-card">
        ...
        {data.history.length ? data.history.slice(0, 4).map((entry) => <HistoryRow key={entry.id} entry={entry} language={language} onCopy={onCopy} onOpen={onOpen} onRetranscribe={onRetranscribe} />) : <EmptyHistory language={language} />}
      </section>
    </div>
  );
}

function FailedCard({ failed, language, onRetry, onDelete }: { failed: FailedRecording; language: AppLanguage; onRetry: () => void; onDelete: () => void }) {
  const t = translator(language);
  return <section className="failed-card"><AlertCircle /><div><strong>{t("failedTitle")}</strong><span>{t("failedBody")}</span><small>{errorMessage(failed.error, language)}</small></div><button className="secondary-button" onClick={onRetry}><RotateCcw />{t("retry")}</button><button className="icon-button danger" onClick={onDelete} aria-label={t("delete")}><Trash2 /></button></section>;
}

function HistoryPage({ history, language, onCopy, onOpen, onRetranscribe, onDelete }: { history: TranscriptEntry[]; language: AppLanguage; onCopy: (text: string) => void; onOpen: (path: string) => void; onRetranscribe: (id: string) => void; onDelete: (entry: TranscriptEntry) => void }) {
  const t = translator(language);
  return <div className="page"><header className="page-header"><div><span className="eyebrow">LOCAL</span><h1>{t("history")}</h1><p>{t("saveHistoryHelp")}</p></div><span className="count-pill">{history.length} / 10</span></header><section className="section-card history-card">{history.length ? history.map((entry) => <HistoryRow key={entry.id} entry={entry} language={language} expanded onCopy={onCopy} onOpen={onOpen} onRetranscribe={onRetranscribe} onDelete={() => onDelete(entry)} />) : <EmptyHistory language={language} />}</section></div>;
}

function HistoryRow({ entry, language, expanded = false, onCopy, onOpen, onRetranscribe, onDelete }: { entry: TranscriptEntry; language: AppLanguage; expanded?: boolean; onCopy: (text: string) => void; onOpen: (path: string) => void; onRetranscribe: (id: string) => void; onDelete?: () => void }) {
  const t = translator(language);
  return <article className={`history-row ${expanded ? "expanded" : ""}`}><div className="history-icon"><AudioLines /></div><div className="history-copy"><p>{entry.text}</p><span><Clock3 />{formatDate(entry.createdAt, language)} · {formatDuration(entry.durationSeconds)} · {modelLabel(entry.model, language)}</span></div><div className="history-actions"><button title={t("copy")} onClick={() => onCopy(entry.text)}><Clipboard /></button>{entry.audioPath && <button title={t("retranscribe")} onClick={() => onRetranscribe(entry.id)}><RotateCcw /></button>}{entry.audioPath && <button title={t("openAudio")} onClick={() => onOpen(entry.audioPath!)}><FileAudio /></button>}{entry.textPath && <button title={t("openText")} onClick={() => onOpen(entry.textPath!)}><FileText /></button>}{onDelete && <button className="danger" title={t("delete")} onClick={onDelete}><Trash2 /></button>}</div></article>;
}
...
function SettingsPage({ data, language, availableUpdate, updateMessage, isBusy, onSave, onRefresh, onCheckUpdates, onInstallUpdate, onToast, onOpenOnboarding }: {
  ...
  return <div className="page settings-page"><header className="page-header"><div><span className="eyebrow">AIDOO WHISPER LITE</span><h1>{t("settings")}</h1><p>{t("version")} {data.appVersion}</p></div><button className="secondary-button" disabled={isBusy} title={isBusy ? t("finishDictationFirst") : undefined} onClick={onOpenOnboarding}><Sparkles />{t("openOnboarding")}</button></header>
    ...
    <SettingsSection icon={<RefreshCw />} title={t("updates")}>
      <SettingRow title={t("launchAtLogin")}><Toggle checked={draft.launchAtLogin} onChange={(launchAtLogin) => setDraft({ ...draft, launchAtLogin })} /></SettingRow>
...
function DeleteDialog({ entry, language, onCancel, onDelete }: { entry: TranscriptEntry; language: AppLanguage; onCancel: () => void; onDelete: (deleteFiles: boolean) => void }) {
  const t = translator(language);
  return <div className="modal-backdrop small"><section className="confirm-dialog"><button className="close-button" onClick={onCancel}><X /></button><span className="danger-icon"><Trash2 /></span><h2>{t("deleteQuestion")}</h2><p>{entry.text}</p><button className="secondary-button" onClick={() => onDelete(false)}>{t("historyOnly")}</button><button className="danger-button" onClick={() => onDelete(true)}>{t("historyAndFiles")}</button></section></div>;
}
