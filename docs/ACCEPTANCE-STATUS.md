...
- Source: parent commit `45c934971a3407f9b68c997bfdb878e7824b7602` (`Keep Lite transcription progress visible`), exported to the dedicated repository as `65beb53fcc0b9e088d56f54d442c116dfc050902`.
- Package: `release/1.0.0/AIDOO Whisper Lite_1.0.0_aarch64.dmg`.
- SHA-256: `5a7e3d4178917cfe8ee44b478dcd19d9ff379112952d2eb29eaae0ffe144f1ea`.
...
- TypeScript checking, all 85 representative English error-localization cases, release configuration, Apple Silicon dependency boundary and all three website-page checks pass.
- The production frontend build passes.
- All 30 native unit tests pass and Rust Clippy passes for all Apple Silicon release targets with warnings denied.
- `npm audit --omit=dev` and the current RustSec database scan report no production vulnerabilities.
- GitHub Actions run [34967682978](https://github.com/bkafelov-aidoo/AIDOO-Whisper-Lite/actions/runs/34967682978) passes checkout, pinned tool setup, both dependency audits, frontend checks/build, all Rust tests and strict Clippy for dedicated-repository commit `65beb53fcc0b9e088d56f54d442c116dfc050902`.
...
- The exact notarized app installed from the final DMG successfully retranscribed an existing 47-second FLAC. The overlay visibly showed `OpenAI is transcribing` with progress, disappeared only after completion, and the app then returned to `Ready for dictation` with a new history item.
...
- Exercise Maximum accuracy and every FLAC/TXT/history toggle combination, output-folder change, history deletion with and without linked files and Launch at Login.
