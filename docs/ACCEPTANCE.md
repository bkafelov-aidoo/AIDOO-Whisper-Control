...
The final `npm run release:mac` path additionally requires a clean Control working tree, current generated third-party notices and the exact `control-vX.Y.Z` tag on HEAD. GitHub Actions accepts the same exact tag and rejects manual branch releases.
...
- Force-quit the app separately while uploading a new dictation, retrying Recovery audio and retranscribing a history FLAC. After each restart, confirm the in-flight audio appears as Delete-only, cannot trigger another OpenAI request, and the original history FLAC remains intact. Force-quit immediately before the request starts and confirm no retryable/non-retryable duplicate files are left behind.
...
- Create a diagnostics package, confirm Finder reveals it, and inspect that it has no API key, transcript text or audio; confirm the separate support button opens a new email to `support@aidoo.bg` without attaching or uploading anything automatically.
