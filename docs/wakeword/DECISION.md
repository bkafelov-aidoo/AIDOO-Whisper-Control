...
Use `livekit-wakeword` 0.1.3 behind AIDOO's own `WakeWordService` boundary. The detector runs entirely on device. The application keeps only a two-second in-memory PCM ring buffer, never writes that buffer to disk, and never sends pre-trigger audio over the network. The classifier is a bundled `hey_aidoo.onnx` resource, while the mel and embedding models are compiled into the Rust dependency.
...
- trigger-to-overlay latency below 500 ms after the phrase ends;
...
After export and evaluation, `scripts/finalize-wakeword-model.py` copies the evaluated ONNX classifier into the application resources and writes `hey_aidoo_model.json`. The command refuses incomplete training splits or evaluation output that lacks the optimal threshold, recall, FPPH, sample counts and validation duration.
