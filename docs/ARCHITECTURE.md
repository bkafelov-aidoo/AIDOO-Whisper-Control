...
The overlay receives state snapshots, progress events and user notices. It never owns recording state. It is centered in the active display's macOS work area, just above the Dock, and grows vertically so long status and error text can wrap. Status and errors are exposed as restrained VoiceOver live regions without announcing every upload-percentage change. Onboarding and deletion dialogs trap keyboard focus, support Escape and restore the previous focus when closed. Readiness requires completed onboarding, an available Keychain key, at least one microphone, Accessibility and no retained failed recording; the main window refreshes this system state when it regains focus. Closing the main window hides it while the tray and global shortcut remain active. A retained failed recording keeps the menu bar in its action-required state until retry or deletion resolves it. A macOS Reopen event restores the main window from the Dock; Quit remains explicit in the menu bar.
