...
The bundle identifier is `app.aidoo.whisper-control`, the minimum system version is macOS 13, and the first release targets Apple Silicon only. The app requests microphone access, Accessibility and optional Apple Events access to Google Chrome so AIDOO Control can reuse the clinic tab and show confirmed changes immediately. Chrome must also have View → Developer → Allow JavaScript from Apple Events enabled so the app can change the route inside the already loaded AIDOO single-page application without triggering a stale service-worker reload. If no clinic tab exists, the app opens the canonical patient URL in Chrome and then falls back to the default browser. It is not App Sandbox constrained because it is distributed outside the Mac App Store.

## Version and local release

Keep the same semantic version in `package.json`, `src-tauri/Cargo.toml` and `src-tauri/tauri.conf.json`. The verified release toolchain is pinned in `rust-toolchain.toml` and both workflows; update it deliberately only after the full source checks pass on the replacement compiler. Update `website/release-notes.html`, regenerate `resources/THIRD_PARTY_NOTICES.txt`, commit, and create the exact `control-vX.Y.Z` tag. The final release command refuses uncommitted Control changes, an untagged HEAD or notices that differ after regeneration. Its preflight also proves that every registered Rust command appears in the application manifest and exactly one window capability boundary. CI rejects a branch run or a tag whose version differs from the application before it imports the signing certificate.
