...
The bundle identifier is `app.aidoo.whisper-control`, the minimum system version is macOS 13, and the first release targets Apple Silicon only. The app requests microphone access, Accessibility and optional Apple Events access to Google Chrome so AIDOO Control can reuse the clinic tab and show confirmed changes immediately. If the user denies Chrome control, the app opens the canonical patient URL without tab reuse. It is not App Sandbox constrained because it is distributed outside the Mac App Store.
